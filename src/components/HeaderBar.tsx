        ...
        <div style={{ position: 'relative' }} ref={ref}>
          ...
          {menuOpen && (
            <div style={{ position: 'absolute', right: 0, marginTop: 8, background: 'white', color: '#0b1724', borderRadius: 8, boxShadow: '0 8px 24px rgba(2,6,23,0.16)', minWidth: 220, zIndex: 60, overflow: 'hidden' }}>
              ...
              <button onClick={() => { setMenuOpen(false); onNavigate('IngresoEfectivo') }} className="btn-opaque" style={{ width: '100%', background: 'transparent', color: '#0b1724', padding: '10px 12px', textAlign: 'left' }}>Movimientos de caja</button>
              ...
              <button onClick={() => { setMenuOpen(false); onNavigate('AnulacionFactura') }} className="btn-opaque" style={{ width: '100%', background: 'transparent', color: '#0b1724', padding: '10px 12px', textAlign: 'left' }}>Anulación de factura</button>
              <button onClick={() => { setMenuOpen(false); setLogoutConfirmOpen(true) }} className="btn-opaque" style={{ width: '100%', background: 'transparent', color: '#0b1724', padding: '10px 12px', textAlign: 'left' }}>Cerrar Sesión</button>
            </div>
          )}
          {logoutConfirmOpen && (
            <div role="dialog" aria-modal="true" className="pv-backdrop" style={{ position: 'fixed', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 12000 }} onClick={() => setLogoutConfirmOpen(false)}>
              <div onClick={e => e.stopPropagation()} className="pv-zoom-modal" style={{ width: 380, background: 'white', borderRadius: 10, padding: 18, boxShadow: '0 18px 40px rgba(2,6,23,0.35)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>

                  <div>
                    <div className="pv-logout-title">Confirmar cierre de sesión</div>
                    <div className="pv-logout-body"> ¿Deseas continuar?</div>
                  </div>
                </div>
                <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 12 }}>
                  <button onClick={() => setLogoutConfirmOpen(false)} className="btn-opaque" style={{ background: 'transparent', padding: '8px 12px', borderRadius: 8 }}>Cancelar</button>
                  <button onClick={() => { setLogoutConfirmOpen(false); try { onLogout() } catch (e) { } }} className="btn-opaque" style={{ background: '#ef4444', color: 'white', padding: '8px 12px', borderRadius: 8 }}>Cerrar sesión</button>
                </div>
              </div>
            </div>
          )}
